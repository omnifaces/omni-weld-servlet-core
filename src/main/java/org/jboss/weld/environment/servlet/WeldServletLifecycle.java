...
public class WeldServletLifecycle {
    ...
    boolean initialize(ServletContext context) {
        ...
        if (isBootstrapNeeded) {
            ...
            /*
             * Determine the BeanManager used for example for EL resolution - this should work fine as all bean archives share the
             * same classloader. The only difference this can make is per-BDA (CDI 1.0 style) enablement of alternatives,
             * interceptors and decorators. Nothing we can do about that.
             *
        ...
        if (Reflections.isClassLoadable(WeldClassLoaderResourceLoader.INSTANCE, JSP_FACTORY_CLASS_NAME) && JspFactory.getDefaultFactory() != null) {
            ...
            // Push the wrapped expression factory into the servlet context so that Tomcat or Jetty can hook it in using a container
            // code
    ...
    protected CDI11Deployment createDeployment(ServletContext context, CDI11Bootstrap bootstrap) {
        ...
        if (isolation == null || Boolean.valueOf(isolation)) {
            ...
            Set<WeldBeanDeploymentArchive> flatDeployment = new HashSet<>();
